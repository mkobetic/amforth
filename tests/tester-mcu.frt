   ...
   depth ACTUAL-DEPTH @ = if		\ IF DEPTHS MATCH
      depth START-DEPTH @ > if		\ IF THERE IS SOMETHING ON THE STACK
         depth START-DEPTH @ - 0 do	\ FOR EACH STACK ITEM
            ...
            <> if s" INCORRECT RESULTS: " ERROR unloop exit then
         loop
      then
   else					\ DEPTH MISMATCH
...
: hardware ( -- )
    ...
    board type
    [char] _ emit
    s" build.rev" environment search-wordlist drop execute type
    [char] _ emit
    s" build.time" environment search-wordlist drop execute type
    cr
;
